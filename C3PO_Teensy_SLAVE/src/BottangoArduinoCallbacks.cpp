...
bool playedOnce = false;  // Add global flag to track if animation 0 has been played.
...
namespace Callbacks
{
    ...
    void onEarlyLoop()
    { 
      // If no animation has been started or looping is enabled and no animation is playing
      if (playOnStart && (!playBegan || (looping && !BottangoCore::commandStreamProvider->streamIsInProgress())))
      {
          BottangoCore::commandStreamProvider->startCommandStream(startingAnimation, looping);
          playBegan = true;
          Serial.print("Playing starting animation for animationIndex: ");
          Serial.println(animationIndex);
      }
      else if (animationIndex != 0 && (!BottangoCore::commandStreamProvider->streamIsInProgress()))
      {
          BottangoCore::commandStreamProvider->startCommandStream(animationIndex, false);
          Serial.print("Playing animation for animationIndex: ");
          Serial.println(animationIndex);
          animationIndex = 0;  // Reset animationIndex after playing
      }
  }
    // void onEarlyLoop()
    // {
    //     // Serial.print("Animation index on early loop: ");
    //     // Serial.println(animationIndex);
    
    //     // Check if animation is still playing
    //     bool animationPlaying = BottangoCore::commandStreamProvider->streamIsInProgress();
    //     // Serial.print("🎥 Animation in progress: ");
    //     // Serial.println(animationPlaying);
    
    //     // Ensure animation 0 only plays ONCE at boot
    //     if (playOnStart && !playedOnce && animationIndex == 0)  
    //     {
    //         BottangoCore::commandStreamProvider->startCommandStream(startingAnimation, looping);
    //         playedOnce = true;  // Set flag to prevent re-triggering animation 0
    //         Serial.println("✅ Playing starting animation ONCE.");
    //     }
    //     // else if (!animationPlaying)  
    //     // {
    //     //     // If no animation is playing, reset animationIndex to -1
    //     //     Serial.println("⏹ No animation playing, resetting animationIndex.");
    //     //     animationIndex = -1;
    //     // }
    
    //     // Debug: Show when a new animation is detected
    //     // if (animationIndex > 0)  
    //     // {
    //     //     Serial.print("🔥 Detected new animationIndex: ");
    //     //     Serial.println(animationIndex);
    //     // }
    
    //     // Ensure animations only play when animationIndex is valid
    //     if (animationIndex > 0 && !animationPlaying)  
    //     {
    // //         Serial.println("🚀 Attempting to start new animation...");
    //         BottangoCore::commandStreamProvider->startCommandStream(animationIndex, false);
    // //         Serial.print("🎬 Playing new animation, animationIndex: ");
    // //         Serial.println(animationIndex);
    //     }
    // }
