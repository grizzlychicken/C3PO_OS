...
#include "GeneratedCode Animations.h"  // Include the generated animation code from Bottango
...
void setup() 
{
    ...
    Serial.println("Bottango setup complete. Right Arm Driver");
}
...
void processSerialCommand(String command) {
  Serial.println("Received command: " + command);  // Debugging print
    if (command.startsWith(":a") || (command.startsWith(":A"))) {
      ...
      BottangoCore::commandStreamProvider.startCommandStream(animationId, false); 
