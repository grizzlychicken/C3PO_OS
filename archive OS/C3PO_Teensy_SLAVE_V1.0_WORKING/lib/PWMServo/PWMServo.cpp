...
// New writeMicroseconds function
void PWMServo::writeMicroseconds(int microseconds)
{
  // Convert microseconds directly to a PWM value
  uint16_t pwmValue = (microseconds * clockCyclesPerMicrosecond()) / 8;
  
  if (pin == SERVO_PIN_A) OCR1A = pwmValue;
  if (pin == SERVO_PIN_B) OCR1B = pwmValue;
  #ifdef SERVO_PIN_C
  if (pin == SERVO_PIN_C) OCR1C = pwmValue;
  #endif
}
...
// New writeMicroseconds function for ARM-based platforms
void PWMServo::writeMicroseconds(int microseconds)
{
  if (pin >= NUM_DIGITAL_PINS) return;
  uint32_t duty = (microseconds * 4096) / 20000;
  analogWriteResolution(12);
  analogWrite(pin, duty);
}
